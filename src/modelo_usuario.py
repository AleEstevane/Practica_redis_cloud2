...
def crear_usuario_json(conexion: Redis, usuario_json: str) -> bool:
    ...
    Devuelve `True` si la clave fue creada (no existía),
...
def leer_usuario_json(
    conexion: Redis, id_usuario: str
) -> Optional[Dict[str, Any]]:
    """
    Lee y devuelve el objeto usuario almacenado en Redis.

    Retorna un diccionario con los datos del usuario o `None`
    si no existe.
    """
    id_usuario = str(id_usuario).strip()
    validar_id_usuario(id_usuario)
...
def actualizar_usuario_json(
    ...
    - `modo` puede ser:
        'mezclar' (fusiona campos)
        'reemplazar' (sustituye todo el objeto)

    Devuelve `True` si la actualización tuvo éxito,
    ...
    if not isinstance(nuevos_datos, dict):
        raise ValueError(
            "El JSON de actualización debe ser un objeto."
        )
    ...
    if modo == "reemplazar":
        ...
        raise ValueError(
            "Modo inválido. Usa 'mezclar' o 'reemplazar'."
        )
...
def eliminar_usuario(conexion: Redis, id_usuario: str) -> bool:
    """
    Elimina el usuario identificado por `id_usuario`.

    Devuelve `True` si la eliminación afectó una clave.
    """
    eliminado = conexion.delete(
        construir_clave_usuario(id_usuario)
    )
    return eliminado == 1
...
def listar_usuarios(
    ...
    """
    claves = sorted(conexion.keys(f"{PREFIJO_USUARIO}*"))
    resultado = []
