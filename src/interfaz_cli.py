...
# del modelo de usuarios usando `obtener_conexion()`.
...
def prompt_json(prompt: str) -> str:
    """
    Solicita al usuario una cadena JSON y la valida.
    """
    ...
    json.loads(texto)  # valida que sea JSON
    return texto
    ...
    while True:
        ...
        try:
            if opcion in ("1", "crear"):
                ...
                modo = (
                    input(
                        "modo (mezclar/reemplazar) [mezclar]: "
                    ).strip()
                    or "mezclar"
                )

                actualizado = actualizar_usuario_json(
                    conexion, idu, datos, modo=modo
                )
                ...
                imprimir(
                    {"total": len(usuarios), "usuarios": usuarios}
                )
